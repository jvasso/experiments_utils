...
def set_experiment_mode(arguments):
    if len(arguments)==0:
        generate_slurm_mode = False
        cluster_mode  = False
    elif len(arguments)==2:
        if arguments[0] == "generate":
            generate_slurm_mode = True
            cluster_mode  = False
        ...
        raise Exception(f'Arguments list {arguments} not supported.')
    names_dict = dict(config_name=config_name, run_name=run_name, group_name=group_name)
    return generate_slurm_mode, cluster_mode, names_dict
...
def single_run(config, train_func:Callable, is_offline, use_wandb:bool, SYNC_WANDB_PATH:str, verbose, run_name:str=None, group_name="basic_training") -> Run:
    """
    ...
    train_func(config=config_object, verbose=verbose, use_wandb=use_wandb)
    return run
...
def run_in_cluster_mode(CONFIGS_PATH:str, SYNC_WANDB_PATH:str, names_dict:str, verbose):
    print('\nRun in cluster mode!')
    ...
    run:Run = single_run(config=single_config, is_offline=True, use_wandb=True, SYNC_WANDB_PATH=SYNC_WANDB_PATH, verbose=verbose, run_name=run_name, group_name=group_name)
    run.finish()